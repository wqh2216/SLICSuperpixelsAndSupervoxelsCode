...
void CSLICSuperpixelsDlg::OnBnClickedButtonCreatesuperpixels()
{
	...
	for( int k = 0; k < numPics; k++ )
	{
		...
		int sz = width*height;//sz=width*height:The total number of pixels in a single image.
		//---------------------------------------------------------
		// m_spcount: is Is the value entered by the client from the interface, ie, 
		//the number of seeds initialized, but the SLIC algorithm may not end up with a 
		//superpixel per seed, as some factors may be merged by other superpixels
		if(m_spcount < 20 || m_spcount > sz/4) m_spcount = sz/200;//i.e the default size of the superpixel is 200 pixels
		//m_compactness:This value is also set by the user, is the tightness ratio between the color features and the XY coordinate features, 
		//20 is often good results.
		if(m_compactness < 1.0 || m_compactness > 80.0) m_compactness = 20.0;
		//---------------------------------------------------------
		//labels£ºint* labels = new int[sz]:A label map, consistent with the image size, 
		//is used to label the label value of each pixel.
		int* labels = new int[sz];
		//numlabels£ºint numlabels(0):Is the number of classes into which the image is finally divided, that is, the number of superpixels
		int numlabels(0); 
		SLIC slic;//slic£ºSLIC slic Is a SLIC class
		slic.DoSuperpixelSegmentation_ForGivenNumberOfSuperpixels(img, width, height, labels, numlabels, m_spcount, m_compactness);//This code is super-pixel generated function entry
		
		 
		
		
		
		//that will eventually be generated, which is initialized to 0 here.
